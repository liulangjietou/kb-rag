...
/**
 * Mints open API keys and reduces a presented key to the form the database stores, requirement section
 * 4.8 "hash storage and display prefix".
 *
 * <p><b>Three forms of one key, on purpose.</b> The plaintext exists only in the creation response, the
 * digest is what authentication compares against, and the display form is what a list page may show. No
 * code path can go from the stored forms back to the plaintext, so a database dump is not a set of usable
 * credentials.
 *
 * <p>Randomness comes from {@link SecureRandom}: a predictable key is the same defect as no key.
 *
...
public class ApiKeyFactory {
    ...
    private static final int DISPLAY_HEAD_LENGTH = KbConstants.API_KEY_PLAINTEXT_PREFIX.length() + 6;

    /** Trailing characters kept in the display form, requirement section 4.8 "prefix plus last four". */
    ...
    public GeneratedKey generate() {
        ...
        String plaintext = KbConstants.API_KEY_PLAINTEXT_PREFIX + toHex(random);
    ...
    /**
     * Tells whether a presented string could be a key of this system at all.
     *
    ...
    public boolean looksLikeKey(String plaintext) {
        return plaintext != null
                && plaintext.startsWith(KbConstants.API_KEY_PLAINTEXT_PREFIX)
