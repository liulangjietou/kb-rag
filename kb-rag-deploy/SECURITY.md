...
- 页面内口令登录必须先完成服务端随机 PNG 拼图并消费短时一次性 proof；横向答案不进入响应，
  校验同时使用服务端实测耗时，verify/login 在 JSON 反序列化前分别受 32KB/8KB 硬上限保护。
  登录失败审计独立提交，成功事务在并发守卫内完成。验证码失败不计入
  账号锁定，具体重放、可信代理绑定与多实例边界见 `docs/LOGIN-CAPTCHA-CONTRACT.md`
- “记住用户名和密码”由浏览器或系统密码管理器保管密码；站点存储只保存用户名和勾选偏好，
  不写入明文密码或前端可解密密文
