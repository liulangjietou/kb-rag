...
- `[schema]` **三张「任意租户可改删全部署数据」的表定性并收口（M16 后修复，Flyway `V23__source_mapping_tenant_and_platform_config.sql`，`docs/M16-CONTRACTS.md` §1.5）**：这三张表都不带租户列，仓库里也从来没有任何一处写下过"它们是有意共享的"。判据是**这份数据是租户的业务资产，还是部署本身的设施**，结论按表分岔——①**聊天导入映射模板收进租户维度**：各租户导出格式本就不同，且四个写端点用的是 `doc:write` 而非 `system:config`，**任何租户的普通文档编辑者都能改删全部署的模板**（别家导入解析随即错乱），破坏面比另外两张更大；加租户列进行级围栏、名称唯一范围由全局收缩为租户内，内置模板改为**建租户时复制一份**（与 V17 复制五个内置角色同一套做法），迁移为已存在的非默认租户补齐。**不做「内置全局 + 租户自建」的混合可见性**——那要在每个查询里写「本租户的 OR 全局的」，围栏表达不了，漏一处就是缺口。启动种子跑在无控制台主体的线程上、围栏整条跳过，因此它的两条语句都显式钉死默认租户（V22 立下的通则）。②③**IK 词典与告警配置登记为部署级全局，收紧到平台运维权限**：词典由 ES 插件按一个 URL 拉取、是集群级设置，按租户切要求每租户一套 analyzer，那不是隔离问题而是另一个特性；告警本就只有一个运维出口，风险在于 `webhook_url` 可被任意租户改成自己的地址、把别家告警内容引出去——**那是「谁能改」的问题，不是「存几份」的问题**。新增第 20 个权限码 `platform:config`（平台级码集合的第二个成员，只授默认租户超管，建租户复制内置角色时自动剔除），与 `tenant:manage` 分开而不是复用它（那个码同时还是用户表与角色表的围栏例外开关，把「改词典」挂上去会让那个例外的边界更难说清）。**行为变更（需要运维知晓）**：子租户超管从此不能改 IK 词典与告警配置，控制台系统设置页的这两个页签对其不再渲染；`system:config` 的其余用途不变。**存量行由列 DEFAULT 划入内置默认租户，单租户部署行为不变**；无新增环境变量与配置键。迁移已在临时库实跑验证（索引收缩、存量行归属、存量租户补齐、幂等重跑 0 行）。同步修订：`M2-CONTRACTS.md` §3、`M3-CONTRACTS.md` §3.6、`M8-CONTRACTS.md` §0.7 各补本域定性说明，OpenAPI `kb-server.yaml` 的 dict / alert 两个 tag 与导入映射列表端点注明（**纯描述订正，schema 与版本号不变**）。

- **开放 API 调用审计查询不带租户（M16 后修复，`docs/M16-CONTRACTS.md` §1.3.2）**：控制台「审计日志查询」的两个端点只按可选的 `key_id` 过滤，**`key_id` 不传就是全部署所有租户的调用流水**——key_id、app_id、endpoint、query 摘要、耗时、错误码。审计行经 `key_id` 归属租户、自己不带租户列，那条语句里没有任何东西能分出租户。**这是解析义务的第三种形态：没有任何 id 可解析**——前两种形态（路径带从属资源 id、路径带 `kbId`）都有一个东西可以拿去解析根表，这里缺省时无从解析，而"没有 id"恰恰等于"全表"。修法反过来：先经行级围栏读出调用者租户下的 API Key 集合，再用它约束查询；**一把都不剩就答空，绝不退化成"不加过滤"**——那会把缺陷原样保留，而且比修复前更难发现，因为代码看起来已经解析过了。传入其他租户的 `key_id` 与传入一个不存在的 key 同解（空结果，不是 403）。审计表**不加租户列**，遵循「从属表经根归属租户」的既有取舍。**无 Flyway 脚本、无新增环境变量与配置键，升级零操作**；写入侧跑在无控制台主体的线程上，行为零变化。同步修订：`M4c-CONTRACTS.md` §3 补本域解析义务说明，OpenAPI `kb-server.yaml` 的 api-audit tag 注明（**纯描述订正，schema 与版本号不变**）。
